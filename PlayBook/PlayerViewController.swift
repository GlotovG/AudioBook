...
//  PlayBook
//
//  Created by Gennadiy Glotov on 08.07.2018.
...
//делаем функцию в UIImageView, для округления элемента интерфейся
...
class PlayerViewController: UIViewController, AVAudioPlayerDelegate {
    ...
    // MARK: - переменный и константы
    
    //переменная для выбора картинки для фона
    let selectedBackground = Int(arc4random_uniform(UInt32(9)))
    
    //переменная для обращения к воспроизводимому файлу
    ...
    //@IBOutlet var songNo : UILabel! //не используется в коде
    //@IBOutlet var lineView : UIView! //не используется в коде
    @IBOutlet weak var albumArtworkImageView: UIImageView!
    @IBOutlet weak var artistNameLabel: UILabel!
    @IBOutlet weak var albumNameLabel: UILabel!
    @IBOutlet var songNameLabel : UILabel!
    //@IBOutlet var songNameLabelPlaceHolder : UILabel! //не используется в коде
    ...
    //@IBOutlet var blurImageView : UIImageView! //не используется в коде
    //@IBOutlet var enhancer : UIView! //не используется в коде
    ...
    //MARK:- Управление воспроизведением на заблокированном экране
    
    // эта функция показывает название воспроизводимого файла на заблокированном экране
    func showMediaInfo(){
    ...
    // эта функция позволяет управлять функциями плеера из заблокированного экрана
    
    override func remoteControlReceived(with event: UIEvent?) {
        //получаем событие и проверяем, что оно принадлежит remoteControl
        if event!.type == UIEventType.remoteControl{
            //выбираем нужное действие при нажатии на кнопку управления на заблокированном экране
            ...
            case UIEventSubtype.remoteControlPreviousTrack:
                previous(self)
            //если пришло иное событие
            default:
                print("Неизвестное событие с управления на заблокированном экране")
            }
        }
    }
...
//    }
    ...
    //функция которая срабатывает при загрузке экрана приложения
    
    override func viewDidLoad() {
        super.viewDidLoad()
        
        //установка фона с указанием местонахождения картинки
        backgroundImageView.image = UIImage(named: "background\(selectedBackground)")
        
        //загрузка и установка последнего прослушиваемого файла
        ...
        // регистрация управления на заблокированном экране
    ...
    //функция установки значения флагов для повторения и перемешивания
    func setRepeatAndShuffle(){
        //загружаем из памяти какие ранее были параметры установлены пользователем
        shuffleState = UserDefaults.standard.bool(forKey: "shuffleState") // получаем статус перемешивания
        repeatState = UserDefaults.standard.bool(forKey: "repeatState") // получаем статус повтора
...
//    }
    ...
    //делаем круглым картинку, Лена не хочет круглую...хочет квадратную...
...
//    }
    ...
    // MARK:- AVAudioPlayer Delegate's обратный метод
    func audioPlayerDidFinishPlaying(_ player: AVAudioPlayer, successfully flag: Bool){
        if flag == true {
            
            if shuffleState == false && repeatState == false {
                // тут нужно добавить проверку включения неприрывного воспроизведения (добавил)
                ...
                //воспроизводим тот же файл что и играл сейчас
                prepareAudio()
                playAudio()
